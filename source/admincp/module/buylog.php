...
<td><?php echo $row['in_tid'] >1 ?$row['in_tid'] >2 ?'包年': '包季': '包月'; ?></td>
<td><?php echo $row['in_money']; ?></td>
<td><?php if($row['in_lock']==0){echo '成功';}else{echo '<em class="lightnum">失败</em>';} ?></td>
<td><a href="?iframe=buylog&action=keyword&key=<?php echo $row['in_uname']; ?>" class="act"><?php echo str_replace(SafeRequest("key","get"),'<em class="lightnum">'.SafeRequest("key","get").'</em>',$row['in_uname']); ?></a></td>
