...
;echo '<tr class="hover">
<td>';echo str_replace(SafeRequest("key","get"),'<em class="lightnum">'.SafeRequest("key","get").'</em>',$row['in_title']);;echo '</td>
<td>';echo $row['in_points'];;echo '</td>
<td>';echo $row['in_money'];;echo '</td>
<td>';if($row['in_lock']==0){echo '成功';}else{echo '<em class="lightnum">失败</em>';};echo '</td>
<td><a href="?iframe=paylog&action=keyword&key=';echo $row['in_uname'];;echo '" class="act">';echo str_replace(SafeRequest("key","get"),'<em class="lightnum">'.SafeRequest("key","get").'</em>',$row['in_uname']);;echo '</a></td>
<td>';if(date('Y-m-d',strtotime($row['in_addtime'])) == date('Y-m-d')){echo '<em class="lightnum">'.$row['in_addtime'].'</em>';}else{echo $row['in_addtime'];};echo '</td>
</tr>
';
}
}
