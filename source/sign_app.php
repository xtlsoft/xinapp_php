	...
	<div class="banner">
		<div class="middle-wrapper clearfix">
			...
			<div class="actions">
				...
				<a class="download ng-binding" style="color:#00BFFF" href="<?php echo getlink($row['in_id']); ?>" target="_blank"><i class="icon-eye"></i> 预览</a>
			</div>
				...
				<div class="app-info-form">
					...
					<div class="field app-short">
						...
						<div class="value actions">
							...
							<button class="save ng-binding" onclick="window.open('<?php echo getlink($row['in_id']);?>')">分发预览</button>
						</div>
					</div>
                    <?php } else {?>
					<div class="field app-short">
					...
					<div class="field app-short">
						...
						<div class="value">
							<div class="apps-app-security">
								<div class="btn-invite-member" style="margin-left:5px;border:1px solid #1E99FF;color:#00BFFF" onclick="layer.prompt({title:'请输入签名密钥'},function(_key){purchase(_key)})">开通或续期</div>
								<div class="btn-invite-member" style="margin-left:5px;border:1px solid #00BFFF;color:#00BFFF" onclick="window.open('<?php echo IN_PATH . 'index.php/sign';?>')"><i class="icon icon-cart"></i> 购买签名密钥</div>
					...
					<div class="field actions">
						<div class="value">
							<div class="apps-app-security">
                                ...
                                <a href="http://wpa.qq.com/msgrd?v=3&uin=16644138&site=qq&menu=yes" target="view_window">自助签名仅支持500MB以下APP，当前APP已超限、需签超限APP请点此联系站长</a></div>';
                                }else{ ?>
                                <text id="in_certs"/>
								<button class="save ng-binding" onclick="sign_confirm();">开始签名</button>
								<?php  } if ($status && $status < 2 && $GLOBALS['erduo_in_userid'] == 1) {//showConfirmDialog('我的对话框','弹框测试');?>
