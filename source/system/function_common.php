...
function getlink($id)
{
    ...
    //$url = IN_REWRITE && $link ? $path . $link : $path . 'app.php/' . auth_codes($id);
    $url = IN_REWRITE && $link ? $path . $link : $path . auth_codes($id);
   return is_ssl() ? str_replace('http://', 'https://', $url) : $url;
     //return is_ssl() ? wodeai(str_replace('http://', 'https://', $url)) : wodeai($url);
}
...
function getavatar($id)
{
  ...
  $ar = explode('@', $mail);
  $QQ = $ar[0];
  $QQ = str_replace('admin','16644138',$QQ);
