...
if ($check) {
    ...
    if ($row && !$roa && $row['shan'] == 0){
        ...
        exit(jiami('1|签名正常！签名请联系QQ:8173816，微信同号！|' . $endate . '|' . $row['in_sign'] . '|' . time() . '|' . $dtime));///联系方式仅为数据加密验证秘钥 不得修改 否则时间锁将会闪退
    }
        $GLOBALS['db']->query("update app set expdate='$endate' where aid=" . $id);
    if ($row['shan'] > 0) {
        exit(jiami('3|此应用已删除，请联系开发者！' . PHP_EOL . PHP_EOL . '10秒后自动闪退！|' . $endate . '|' . $row['in_sign'] . '|联系QQ:8173816，微信同号|' . $dtime));///联系方式仅为数据加密验证秘钥 不得修改 否则时间锁将会闪退
    }
    if ($roa && $roa['isuse'] == 0) {
        exit(jiami('3|' . $roa['msg']. PHP_EOL . '10秒后自动闪退！|' . $endate . '|' . $row['in_sign'] . '|联系QQ:8173816，微信同号|' . $dtime));///联系方式仅为数据加密验证秘钥 不得修改 否则时间锁将会闪退
    }
    if ($row['in_sign'] > time()) {
        exit(jiami('1|签名正常！签名请联系QQ:8173816，微信同号！|' . $row['in_bsvs'] . '|'. $roa['j_url'] . '|' . $endate . '|' . $row['in_sign'] . '|' . time() . '|' . $dtime));///联系方式仅为数据加密验证秘钥 不得修改 否则时间锁将会闪退
    }
    ...
    if (time() - $row['in_sign'] < 3600 && $row['in_sign'] < time()) {
        ...
        exit(jiami('2|' . $roa['msg']. PHP_EOL . '点OK可暂时使用！|' . $endate . '|' . $row['in_sign'] . '|联系QQ:8173816，微信同号|' . $dtime));///联系方式仅为数据加密验证秘钥和版权 不得修改 否则时间锁将会闪退
    }
    if ($row['in_sign'] < time()) {
        $GLOBALS['db']->query("update app set isuse='0',state='2' where aid=" . $id);
        exit(jiami('3|' . $roa['msg']. PHP_EOL . '10秒后自动闪退！|' . $endate . '|' . $row['in_sign'] . '|联系QQ:8173816，微信同号|' . $dtime));///联系方式仅为数据加密验证秘钥和版权 不得修改 否则时间锁将会闪退
    }
}
function jiami($str)
{
