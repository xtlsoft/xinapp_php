function upload_a_icon() {
    ...
    if (in_login < 1) {
        $("#tips_a_icon").text("请先登录");
        return false;
    }
    if (upfile.size > 1048576) {
        $("#tips_a_icon").text("图标不能大于1M");
        return false;
    }
    var fd = new FormData();
...
function upload_l_image1() {
    var upfile = $("#upload_l_image1")[0].files[0];
    if (in_login < 1) {
        $("#tips_l_image1").text("请先登录");
        return false;
    }
    if (upfile.size > 2097152) {
        $("#tips_l_image1").text("图片不能大于2M");
        return false;
    }
    var fd = new FormData();
    fd.append("webview", upfile);
    var l_image_xhr = new XMLHttpRequest();
    l_image_xhr.open("post", in_path + "source/pack/webview/ajax.php");
    l_image_xhr.onload = complete_l_image1;
    l_image_xhr.onerror = failed_l_image1;
    l_image_xhr.upload.onprogress = progress_l_image1;
    l_image_xhr.send(fd);
}

function progress_l_image1(evt) {
    var per = Math.round(evt.loaded / evt.total * 100);
    $("#tips_l_image1").text(per + "%");
    if (per > 99) {
        $("#tips_l_image1").text("请稍等...");
    }
}

function complete_l_image1(evt) {
    var response = evt.target.responseText;
    if (response == "return_0") {
        $("#tips_l_image1").text("文件不规范");
    } else {
        $("#preview_l_image1").html('<img width="200" height="200" src="' + in_path + "data/tmp/" + response + '">');
    }
}
function failed_l_image1() {
    $("#tips_l_image1").text("上传异常");
}
