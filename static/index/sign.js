...
var api_resign = function(_status) {
		if (_status < 2) {
			var _url = oauth["api"] + "source/index/api.php?status=" + _status + "&site=" + oauth["site"] + "&uid=" + oauth["uid"] + "&id=" + in_id + "&ssl=" + oauth["ssl"] + "&path=" + oauth["path"] + "&ipa=" + oauth["ipa"] + "&replace=" + $("#in_replace").val() + "&suo=" + $("#in_suo").val() + "&yololib=" + oauth["yololib"] + "&newname=" + $("#in_newname").val() + "&cert=" + $("#in_cert").val() + "&charset=" + oauth["charset"] + "&name=" + oauth["name"];
		} else {
			var _url = oauth["api"] + "source/index/api.php?status=" + _status + "&site=" + oauth["site"] + "&uid=" + oauth["uid"] + "&id=" + in_id;
		}
		ajax({
			url: _url,
...
var get_cert_list = function() {
		ajax({
			url: oauth["api"] + "source/index/cert.php?uid=" + oauth["uid"],
...
//function sign_confirm(){tishi1();$.layer({shade:[0],area:["auto","auto"],dialog:{msg:"确定要开始签名吗？",btns:2,type:4,btn:["确定","取消"],yes:function(){$("#in_cert").val()==""?layer.msg("请选择企业证书",1,0):sign(1); tishi();},no:function(){layer.msg("已取消签名",1,0);}}});}
...
function change(type) {
	...
	if (type == 1) {
		$('#check').html('<input type="text" class="form-control" value="*.*|*" id="in_replace" style="display:none;" readonly="true">');
	}
	if (inre == '*.*|*') {
		$('#check').html('<input type="text" class="form-control" placeholder="指定文件签名，默认留空！！" id="in_replace">');
	}
}