...
function reg() {
	...
	*/
	...
	//xhr.open("GET", in_path + "source/index/ajax.php?ac=reg&mail=" + mail.value + "&mcode=" + mcode.value + "&pwd=" + rpwd.value + "&seccode=" + seccode.value, true);
...
function sends_mail() {
	...
	var mail = document.getElementById("mail");
	document.getElementById("alert-warning").style.display = "block";
	if (mail.value.indexOf("qq") == -1) {
		document.getElementById("alert-warning").innerHTML = "<ul><li>请使用数字QQ邮箱注册！<br>例如：123456@qq.com</li></ul>";
		mail.focus();
		return;
	}
	if (strLen(mail.value) < 1 || isEmails(mail.value) == false) {
		document.getElementById("alert-warning").innerHTML = "<ul><li>邮箱格式有误，请更改！<br>请用数字QQ邮箱，例如：123456@qq.com</li></ul>";
		mail.focus();
		return;
	}
	document.getElementById("send_btn").innerHTML = "获取中...";
	xhr.open("GET", in_path + "source/index/ajax.php?ac=sends&mail=" + mail.value, true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
			if (xhr.status == 200) {
				if (xhr.responseText == "return_0") {
					...
					document.getElementById("alert-warning").innerHTML = "<ul><li>用户已存在，请更改！</li></ul>";
				} else if (xhr.responseText == "return_2") {
					document.getElementById("alert-warning").innerHTML = "<ul><li>请等待 30 秒后再重新获取！</li></ul>";
				} else if (xhr.responseText == "return_4") {
					document.getElementById("alert-warning").innerHTML = "<ul><li>邮件码已发送至邮箱，请注意查收！</li></ul>";
...
function isEmails(input) {
	if (input.match(/^([0-9_\.\-])+\@(([a-zA-Z0-9\-])+\.)+([a-zA-Z0-9]{2,4})+$/)) {
		return true;
	}
	return false;
}
